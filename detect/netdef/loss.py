...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if self.num_hard > 0 and train:
            neg_output, neg_labels = hard_mining(neg_output, neg_labels, self.num_hard * batch_size)  # 只选择置信度较高的负样本作计算
        neg_prob = self.sigmoid(neg_output)  # TODO
...
if __name__ == '__main__':
    net = Loss()
    dummy = torch.randn(1, 2, 2, 2, 3, 5)
    labels = torch.randn(1, 2, 2, 2, 3, 5)
    net(dummy, labels)
