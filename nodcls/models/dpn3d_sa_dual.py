...
class DPN(nn.Module):
    def __init__(self, cfg):
        ...
        self.sa = Self_Attn3D(2560, 2560 // 8, 2560 // 8)
        self.conv_sa1 = nn.Sequential(
            nn.Conv3d(2560, 2560 / 2, kernel_size=1),
            nn.BatchNorm3d(2560 / 2)
        )
        self.conv_sa2 = nn.Sequential(
            nn.Conv3d(5120, 2560 / 2, kernel_size=1),
            nn.BatchNorm3d(2560 / 2))
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out_s1, out_s2 = self.sa(out)
        out_s1 = self.conv_sa1(out_s1)
        out_s2 = self.conv_sa2(out_s2)
        out = torch.cat((out_s1, out_s2), 1)
